...
b_p = tf.nn.relu(a_p)
c_p = tf.nn.bias_add(b_p, i_p, data_format='NHWC')
d_p = tf.nn.conv2d(c_p, w_p, strides=[1, 1, 1, 1], padding='SAME', data_format='NHWC')

with tf.device("/CPU:0"):
    b = tf.nn.relu(a)
    c = tf.nn.bias_add(b, i, data_format='NHWC')
    d = tf.nn.conv2d(c, w, strides=[1, 1, 1, 1], padding='SAME', data_format='NHWC')
